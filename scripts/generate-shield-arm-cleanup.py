...
def generate(seed, lower_shoulder=False):
    part = (lowered_shoulder if lower_shoulder else revised_part)(catalog(), seed)
    write_definition(part, ROOT/'fdm_sculpt/components/parts')
    golden = ROOT/('tests/fixtures/lowered-shoulder-golden.json' if lower_shoulder else 'tests/fixtures/shield-arm-cleanup-golden.json')
    ...
    for placement in assembly['placements']:
        if placement['part'] == ('aurelian.left-arm@4' if lower_shoulder else 'aurelian.left-arm@3'):
...
if __name__ == '__main__':
    ...
    parser.add_argument('--lower-shoulder', action='store_true')
    args=parser.parse_args()
    generate(args.seed,args.lower_shoulder)
