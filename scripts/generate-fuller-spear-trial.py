"""Publish the thicker decorative spearhead trial as immutable component revisions."""
import json
import sys
...
figures = [json.loads((ROOT/f'specs/experiments/decorative-spear-infantry-{i}-trial.json').read_text())
           for i in range(1, 6)]
parts, specs = build(figures, catalog(), thicker=True)
for part in parts:
...
(ROOT/'tests/fixtures/fuller-spear-golden.json').write_text(
