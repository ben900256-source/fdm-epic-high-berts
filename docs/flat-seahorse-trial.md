...
Use `specs/experiments/raised-flat-seahorse-infantry-{1..5}-trial.json` for the
next trial. Generate with `py -3.13 scripts/generate-raised-flat-seahorse.py`.
...
Use the raised-flat-seahorse figure recipes above for the next shield trial with
the current Cool Detail v1 print and filament presets.
