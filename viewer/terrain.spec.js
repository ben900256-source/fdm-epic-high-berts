...
test('current terrain reviews load and retain body/surface identification',async({page})=>{
  ...
  for(const entry of index.reviews){
    ...
    if(!['-terrain-v3','-natural-ground-v1','-visible-soil-v1','-fitted-bow-grips-v1'].some(suffix=>manifest.build.endsWith(suffix))&&entry.id!=='aurelian-terrain-gallery')continue;
