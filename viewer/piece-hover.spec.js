...
test('Ctrl shows the exact piece and component together; Alt remains an alias',async({page})=>{
  ...
  await page.selectOption('#figure','elf-01');
  await page.selectOption('#part','aurelian.left-leg@6');
  ...
  for(let y=250;y<900&&roles.size<2;y+=40)for(let x=500;x<1060&&roles.size<2;x+=40){
    ...
    if(role){
      roles.add(role);
      await expect(page.locator('body')).toHaveAttribute('data-hovered-piece','elf-01/left-leg');
      await expect(page.locator('#piece-reference')).toHaveText('aurelian.left-leg@6');
      ...
      expect(resolved.instance_id).toBe('elf-01/left-leg');
      expect(resolved.geometry_role).toBe(role);
      expect(resolved.part).toBe('aurelian.left-leg@6');
    }
  }
