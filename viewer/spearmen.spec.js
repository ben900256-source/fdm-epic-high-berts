...
test('ten spearmen, hawk sergeant and aligned horn hand load their current revisions',async({page})=>{
  ...
  for(const [id,build] of [
    ['aurelian-spearman-variants','elf-spearman-variants-v2'],
    ['aurelian-spearman-hawk-sergeant','elf-spearman-hawk-sergeant-v2'],
    ...
    if(id==='aurelian-spearman-variants'){
      ...
      expect(manifest.assembly.placements.filter(p=>p.part==='aurelian.shortblade@1')).toHaveLength(4);
    }
